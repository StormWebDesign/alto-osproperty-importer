...
// Define the directory for full property XML exports
$fullPropertyXmlExportDir = __DIR__ . '/xml/full_properties/';
if (!is_dir($fullPropertyXmlExportDir)) {
    mkdir($fullPropertyXmlExportDir, 0755, true); // Create directory if it doesn't exist
...
if (count($pendingProperties) > 0) {
    ...
    foreach ($pendingProperties as $propertyRow) {
        ...
        if ($fullPropertyXml) {
            // Save the full property XML to the designated directory
            $fullXmlFilePath = $fullPropertyXmlExportDir . $altoPropertyId . '.xml';
            if (file_put_contents($fullXmlFilePath, $fullPropertyXml) !== false) {
                Logger::log("        Full XML for property " . $altoPropertyId . " saved to: " . $fullXmlFilePath, 'INFO');
            } else {

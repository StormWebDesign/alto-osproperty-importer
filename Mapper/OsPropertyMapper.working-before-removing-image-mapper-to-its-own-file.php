...
class OsPropertyMapper
{
    ...
    public static function mapPropertyDetailsToDatabase(\SimpleXMLElement $propertyXmlObject, $altoBranchId)
    {
        ...
        try {
            ...
            // ------------------------------------------------------------
            // NEW: Fetch <lastchanged> timestamp from alto_properties.xml_data
            // ------------------------------------------------------------
            $lastChanged = null;
    ...
    ");
                ...
                if ($xmlRaw) {
                    ...
                    if ($xmlObj && isset($xmlObj->lastchanged)) {
                        ...
                        // Strip milliseconds: 2025-08-08T13:32:31.45 → 2025-08-08T13:32:31
                        $clean = preg_replace('/\.\d+$/', '', $raw);

                        // Replace T with space: 2025-08-08 13:32:31
            ...
            // If not available, fallback to NOW()
            if (!$lastChanged) {
            ...
            // Convert for OS Property DATE columns (YYYY-MM-DD)
